...
export class CampaignAccessService {
  ...
  async assignUserToCampaign(userId: number, campaignId: number): Promise<void> {
  ...
  async removeUserFromCampaign(userId: number, campaignId: number): Promise<void> {
  ...
  async userHasAccess(userId: number, campaignId: number): Promise<boolean> {
    const stmt = this.db.prepare(`
  ...
  async getUserCampaigns(userId: number): Promise<number[]> {
    const stmt = this.db.prepare(`
  ...
  async getCampaignUsers(campaignId: number): Promise<number[]> {
    const stmt = this.db.prepare(`
      SELECT user_id FROM campaign_users
      WHERE campaign_id = ?
      ORDER BY created_at DESC
    `)
    const results = await stmt.all(campaignId) as Array<{ user_id: number }>
    return results.map(r => r.user_id)
  }

  // Assign multiple users to a campaign
  async assignUsersToCampaign(userIds: number[], campaignId: number): Promise<void> {
  ...
  async assignUserToCampaigns(userId: number, campaignIds: number[]): Promise<void> {
  ...
  async removeUserFromAllCampaigns(userId: number): Promise<void> {
  ...
  async getUserCampaignCount(userId: number): Promise<number> {
